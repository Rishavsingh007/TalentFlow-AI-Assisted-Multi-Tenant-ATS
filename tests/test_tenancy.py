...
def test_acme_user_gets_404_on_globex_application_detail(api_client):
    ...
    response = api_client.get(f"/api/v1/companies/globex/applications/{globex_application.id}/")
...
def test_same_email_applications_isolated_per_tenant(api_client, settings, tmp_path):
    ...
    acme_app = Application.objects.get(
        company=acme_member.company, applicant_email="shared@example.com"
    )
    globex_app = Application.objects.get(
        company=globex_member.company, applicant_email="shared@example.com"
    )

    _auth(api_client, acme_member)
    acme_detail = api_client.get(f"/api/v1/companies/acme-iso/applications/{acme_app.id}/")
    assert acme_detail.status_code == 200
    assert acme_detail.json()["candidate"]["full_name"] == "Shared Email Acme"

    globex_detail = api_client.get(f"/api/v1/companies/globex-iso/applications/{globex_app.id}/")
